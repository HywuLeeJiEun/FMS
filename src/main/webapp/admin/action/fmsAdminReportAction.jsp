	...
	if(list.size() == 0) {
		...
		for(int i=0; i < list.size(); i++) {
			if(fms_day.equals("fms_rec")) {
				...
				rec.remove(fms_recm);
			} else if(fms_day.equals("fms_str")) {
				int fms_recm = Integer.parseInt(list.get(i).getFms_str().split("-")[1]); // ex> 5;
				rec.remove(fms_recm);
			} else {
				int fms_recm = Integer.parseInt(list.get(i).getFms_end().split("-")[1]); // ex> 5;
				rec.remove(fms_recm);
			}
		}
		...
		for(int i=0; i < wb.getNumberOfSheets(); i++) {
			...
			if(sn.contains("월")) {
				...
				ArrayList<fmsrept> flist = fms.getExcelMMfms(fms_day, year+"-"+mm);
				fovAll = 0;
		...
		String filename = "장애리포트_"+year+".xlsx";
