...
<%@page import="org.apache.commons.text.similarity.LevenshteinDistance"%>
...
<%
	...
	String filePath = "C:\\Users\\gkdla\\git\\FMS\\src\\main\\webapp\\WEB-INF\\fault_reports\\FMSFile\\migration\\"+fileName;
	    		...
	    		for(Cell cell : row) {
	    			if(cell.getColumnIndex() == getColDoc) {
	    				...
	    				for(String con : conlist) {
	    					int maxLen = fms_con.length() > con.length() ? fms_con.length() : con.length();
	    					
	    					LevenshteinDistance ld = new LevenshteinDistance();
		    				
		    				double temp = ld.apply(fms_con, con);
		    				result = (maxLen - temp) / maxLen; 		    				
		    				if(result > 0.7) {
		    					// 70퍼 이상의 정확도라면, (이미 데이터가 있음!)
		    					forNum = -1;
		    					break;
		    				}
	    				}	
	    			...
	    			} else if(cell.getColumnIndex() == getColSig) {
	    				if(cell.toString().contains("종료")) {
	    					fms_sig = "제출";
	    				} else {
	    					fms_sig = "저장";
	    				}
